package io.golos.cyber4j.chain.actions.query.proxy
...
class GetRegProxyInfo(
    ...
    fun getProxies(limit: Int, nextAccount: String = ""): Single<List<io.golos.cyber4j.chain.actions.query.proxy.ProxyJson>> {
        return chainApi.getTableRows(GetTableRows(
            ...
            if (response.isSuccessful) {
                response.body()!!.rows.map { row ->
                    io.golos.cyber4j.chain.actions.query.proxy.ProxyJson(
                            row["owner"].toString(),
                            row["name"].toString(),
                ...
                throw io.golos.cyber4j.chain.actions.query.proxy.GetRegProxyInfo.FailedToFetchProxies()
            }
        }
    }

    fun getProxy(accountName: String): Single<io.golos.cyber4j.chain.actions.query.proxy.ProxyJson> {
        return chainApi.getTableRows(GetTableRows(
                "regproxyinfo",
                "regproxyinfo",
                "proxies",
